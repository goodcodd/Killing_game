...
const westedElement = document.querySelector('.wasted');
...
function init() {
    ...
    player = new Player(canvas.width/2, canvas.height/2, context, movementLimit);
    addEventListener("click", creatProjectile);
}

function creatProjectile(event) {
    projectiles.push(
        new Projectile(
            player.x,
            player.y,
...
function spawnEnemies() {
    ...
    spawIntervaiId = setInterval(() => enemies.push(new Enemy(canvas.width, canvas.height, context, player)), 1000);
...
function animate() {
    ...
    particles = particles.filter(particles => particles.alpha > 0);

    const isGameOver = enemies.some(checkHittingPlayer);
    if (isGameOver) {
        westedElement.style.display = 'block';
        clearInterval(countIntervalId);
        clearInterval(spawIntervaiId);
...
function checkHittingEnemy(enemy) {
    projectiles.some((projectiles, index) => {
        const distance = distanceBetweenTwoPoints(projectiles.x, projectiles.y, enemy.x, enemy.y);
        if (distance - enemy.radius - projectiles.radius > 0) return false;
